...
> 日期：2026-09-01
> 状态（2026-09-25）：Release A 已实施；Release B 的结构化解析、后三首队列预检及逐首状态已实现；Release C 的音源健康度与网易云代理 4.32.0 升级已实现，完整权益/版本诊断页与自动播放恢复中的 YouTube 后台候选仍未实现；显式搜索页已实现快速源先返回、慢源后台补充及请求代次隔离。
> 优先级：P0
...
- **本地曲库与 OpenSubsonic 现已排在实验性在线源前，并可在设置中排序。** 第三方私有接口作为尽力而为的在线补充。
...
2026-09-25 增量实施：`IMusicProvider.ResolveAsync` 统一返回 `MediaResolutionResult`；播放开始后并发预检后续最多 3 首并显示分类状态，切歌/凭据变化会取消旧预检；设置页诊断增加最近 20 条健康记录中的正常接口响应数、搜索/实际播放解析成功时间和熔断冷却（登录/版权限制只算接口正常响应，不算解析成功）；网易云代理锁定 4.32.0 并更新兼容范围内的传递依赖。显式搜索页的慢源后台补充与请求代次隔离已完成。完整账号权益/设备/版本更新诊断、自动播放恢复中的 YouTube 后台候选以及真实设备验收仍未完成。

连接诊断最初于 2026-09-15 交付；2026-09-25 增加本地健康度快照，但不推断上游会员权益、设备风控状态或可用更新。当前 `npm audit --omit=dev` 对网易云代理仍报告 `music-metadata`/`file-type` 链 3 项告警（1 中危、2 高危），不能把 vendor 升级等同于供应链漏洞全部消除。
...
2026-09-25：本地曲库与 OpenSubsonic 已注册到 Broker，默认排在实验性在线源前；用户可在设置页启停并调整优先级。下表其余账号与版权策略仍是目标口径，不能把搜索成功视为最终播放授权。

| 音源 | 新定位 | 默认状态 | 处理 |
|---|---|---:|---|
| 本地曲库 | 稳定主源 | 启用 | 默认优先，用户可排序 |
...
- 显式搜索页在快速源无结果时后台请求慢源；更改搜索词、重新搜索或选择歌曲会取消旧请求，请求代次阻止迟到结果回写。自动播放恢复尚不启动慢源，恢复链路的后台候选仍待独立实现。
- 浏览器 Cookie 仍由用户显式启用。
...
- 已将 `NeteaseCloudMusicApi` 从锁定的 4.31.0 升级并固定到 4.32.0；完成可复现依赖安装、接口导出检查与编译测试，上游在线播放仍待真实环境验收。
- 继续拒绝把试听流冒充完整歌曲。
